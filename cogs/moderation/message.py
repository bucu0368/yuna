...
async def do_removal (ctx ,limit ,predicate ,*,before =None ,after =None ):
  ...
  temp_msg = await ctx.reply(view=temp_view)
...
class PurgeAmountModal(ui.Modal, title="Purge Amount"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.purge_type == "all":
            ...
            if amount > 2000:
                return await self.view.ctx.reply(f"Too many messages to search for ({amount}/2000)")
            ...
            await self.view.ctx.reply(f"✅ | Successfully removed {total_reactions} reactions.", delete_after=7)
...
class Message (commands .Cog ):
  ...
  async def clear (self ,ctx ,choice :Union [discord .Member ,int ]=None ,amount :int =None ):
        if choice is None:
            ...
            processing_message = await ctx.reply(view=loading_view)
        ...
        if isinstance(choice, discord.Member):
            ...
            if limit < 1:
                return await ctx.reply("Message limit must be at least 1")
            return await do_removal(ctx, limit, lambda e: e.author == choice)
            
        elif isinstance(choice, int):
            if choice > 2000:
                return await ctx.reply(f"Too many messages to search given ({choice}/2000)")
            if choice < 1:
                return await ctx.reply("Message limit must be at least 1")
