...
class GuildProfile(Cog):
    ...
    async def setguildavatar(self, ctx: Context):
        ...
        if not ctx.message.attachments:
            return await ctx.reply("Please attach an image while using this command.")
        
        attachment = ctx.message.attachments[0]
        
        if not attachment.content_type or not attachment.content_type.startswith('image/'):
            return await ctx.reply("Please attach a valid image file.")
        ...
        try:
            ...
            await ctx.reply(f"Failed to update avatar: {str(e)}")
    ...
    async def setguildbanner(self, ctx: Context):
        ...
        try:
            ...
            await ctx.reply(f"Failed to update banner: {str(e)}")
        except Exception as e:
            await ctx.reply(f"An error occurred: {str(e)}")
    ...
    async def setguildbio(self, ctx: Context, *, bio: str):
        ...
        try:
            ...
            await ctx.reply(f"Failed to update bio: {str(e)}")
        except Exception as e:
            await ctx.reply(f"An error occurred: {str(e)}")
    ...
    async def setguildname(self, ctx: Context, *, nickname: str):
        ...
        if len(nickname) > 32:
            return await ctx.reply("Nickname must be 32 characters or less.")
        ...
        try:
            ...
            await ctx.reply(f"Failed to update nickname: {str(e)}")
        except Exception as e:
            await ctx.reply(f"An error occurred: {str(e)}")
    ...
    async def setguildclear(self, ctx: Context):
        ...
        try:
            ...
            await ctx.reply(f"Failed to reset profile: {str(e)}")
        except Exception as e:
            await ctx.reply(f"An error occurred: {str(e)}")
