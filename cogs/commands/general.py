...
class PollView(discord.ui.LayoutView):
    def __init__(self, question: str, author: discord.Member):
        ...
        self.author = author
        self.yes_voters: set = set()
        self.no_voters: set = set()

        self.container = discord.ui.Container(accent_color=None)

        self._yes_btn = discord.ui.Button(
            label="✅ Yes",
            style=discord.ButtonStyle.success,
            custom_id="poll_yes"
        )
        self._yes_btn.callback = self._yes_callback

        self._no_btn = discord.ui.Button(
            label="❌ No",
            style=discord.ButtonStyle.danger,
            custom_id="poll_no"
        )
        self._no_btn.callback = self._no_callback

        self._button_row = discord.ui.ActionRow(self._yes_btn, self._no_btn)
    ...
    def _build_layout(self):
        ...
        bar_width = 10
        if total > 0:
            yes_filled = round((yes_count / total) * bar_width)
            no_filled = round((no_count / total) * bar_width)
        else:
            yes_filled = 0
            no_filled = 0

        yes_bar = "█" * yes_filled + "░" * (bar_width - yes_filled)
        no_bar = "█" * no_filled + "░" * (bar_width - no_filled)

        self.container.add_item(discord.ui.TextDisplay("# 📊 Poll"))
        self.container.add_item(discord.ui.Separator())

        poll_text = (
            f"**{self.question}**\n\n"
            f"✅ **Yes** — {yes_count} vote(s)\n"
            f"`{yes_bar}`\n\n"
            f"❌ **No** — {no_count} vote(s)\n"
            f"`{no_bar}`\n\n"
            f"*Total votes: {total}*"
        )
        ...
        self.container.add_item(
            discord.ui.Section(
                discord.ui.TextDisplay(poll_text),
                accessory=discord.ui.Thumbnail(avatar_url)
            )
        )

        self.container.add_item(discord.ui.Separator())
        self.container.add_item(self._button_row)
...
class General(commands.Cog):
    ...
    async def poll (self ,ctx : commands .Context ,*,message ):
        poll_view = PollView(question=message, author=ctx.author)
        await ctx.reply(view=poll_view)
    ...
    async def wizz (self ,ctx : commands .Context ):
        ...
        processing_msg = await ctx.reply(view=loading_view)
    ...
    async def invite (self ,ctx : commands .Context ):
        ...
        footer_info = f"Developed by **GhostX Development**"
        ...
        try:
            ...
            async with self.aiohttp.get(tts_url) as response:
                if response.status == 200:
                    ...
                    await ctx.reply(file=audio_file)
                else:
