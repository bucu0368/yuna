...
class ProfilePictures(commands.Cog):
    ...
    async def anime(self, ctx):
        if not self.anime_pfps:
            await ctx.reply("No anime pfps available.")
            return
        ...
        msg = await ctx.reply(view=initial_view)
    ...
    async def male(self, ctx):
        if not self.male_pfps:
            await ctx.reply("No male pfps available.")
            return
    ...
    async def female(self, ctx):
        if not self.female_pfps:
            await ctx.reply("No female pfps available.")
            return
    ...
    async def matching(self, ctx):
        if not self.matching_pfps:
            await ctx.reply("No matching pfps available.")
            return
