...
class Owner (commands .Cog ):
    ...
    async def adminlock (self ,ctx :Context ):
        """Toggle adminlock - only usable by bot owner"""
        owner_ids = list(self.client.owner_ids) if hasattr(self.client, 'owner_ids') and self.client.owner_ids else OWNER_IDS
        ...
        try :
            ...
            container .add_item (ui .TextDisplay (f"**Admin Lock {status}**\n\nAll commands are now {'locked for non-owners' if new_state else 'unlocked for everyone'}."))
