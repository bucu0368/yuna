...
class Yuna (commands .AutoShardedBot ):
    ...
    async def set_streaming_status (self ):
        ...
        activity =discord .Streaming (
            name =f"{bot_prefix}help | GhostX Development",
            url ="https://twitch.tv/ghostx"
        )
    ...
    **kwargs )->typing .Optional [discord .Message ]:
        ...
        if message .guild :
            ...
            async with aiosqlite .connect ('db/np.db')as db :
                async with db .execute ("SELECT id FROM np WHERE id = ?",(message .author .id ,))as cursor :
                    ...
                    if row :
                        ...
                        prefix =data ["prefix"]
                        return commands .when_mentioned_or (prefix ,bot_prefix ,'')(self ,message )
                    else :
                        data =await getConfig (guild_id )
                        prefix =data ["prefix"]
                        return commands .when_mentioned_or (prefix ,bot_prefix )(self ,message )
            ...
            async with aiosqlite .connect ('db/np.db')as db :
                async with db .execute ("SELECT id FROM np WHERE id = ?",(message .author .id ,))as cursor :
                    row =await cursor .fetchone ()
                    if row :
                        return commands .when_mentioned_or (bot_prefix ,'')(self ,message )
                    else :
                        return commands .when_mentioned_or (bot_prefix )(self ,message )
    ...
    async def global_adminlock_check(self, ctx):
        ...
        owner_ids = list(self.owner_ids) if self.owner_ids else OWNER_IDS
        ...
        if is_locked:
            ...
            container.add_item(ui.TextDisplay(f"sybau {ctx.author.mention} you're not allowed to use it! (Global Lock Active)"))
